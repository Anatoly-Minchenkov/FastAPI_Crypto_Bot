...
@db_session
def create_wallet(user: pydantic_models.User = None, private_key: str = None, testnet: bool = True): #True/false, если тестовый кошелёк
...
def create_transaction(
        ...
        fee: float | None = None,
        testnet: bool = True #Поменять на False
):
    """
    ...
    transaction = Transaction(sender=sender,
                              ...
                              tx_hash=tx_hash)
    sender.wallet.send([f'{receiver_address}, {amount_btc_without_fee}, btc']) #это я сам дописал, как и всю эту ф-ию
