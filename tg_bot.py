...
def wallet(message):
    ...
    markup.add(btn1)
    if wallet["balance"]:
        text = f'Ваш баланс: {wallet["balance"] / 100000000} BTC\n Ваш адрес: {wallet["address"]}'
    else:
        text = f'Ваш баланс: 0 BTC\n Ваш адрес: {wallet["address"]}'
    bot.send_message(message.chat.id, text, reply_markup=markup)
...
def history(message):
    ...
    transactions = client.get_user_transactions(
        client.get_user_by_tg_id(message.from_user.id)['id'])  # загружаем транзакции
...
@bot.message_handler(func=lambda message: message.from_user.id == tg_admin_id and message.text == "Общий баланс")
